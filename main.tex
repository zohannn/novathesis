...
\documentclass[
  ...
  school=um/eeng,      % nova/fct(*), nova/fcsh, nova/ims, ul/ist, ul/fc, ipl/isel
  lang=en,              % en(*), fr, it, pt
  %coverlang=en,         % defaults to main language
  %copyrightlang=en,     % defaults to main language
  fontstyle=newsgott,    % baskervaldx bookman charter ebgaramond fbb fourier garamond
...
% Important package for the licensing. By default english is used, however
% is necessary to explicity define the language if the license is in
% portuguese. The modifier must be changed accordingly to user preferences.
% If the licensing is different from any of the Creative Commons (all
% rights reserved for instance) comment the line with \printcopyright and
% add your own custom file in that place. Note that all files must be in
% the Chapters folder to include.
